...
/*
 Q&D EXT2 filesystem just for testing.
*/
...
class FS {
  public:
    ...
    void* kfopen(const char* path){
      // Since we are testing for "initd" process,
      // we will only search for this file at root.
      
      for(int i = 0; i<m_.s_inodes_count; i++) {
        ext2_inode _inode = m_root_inode
      }
    }
};
